<template>
    <layout-children>
        ...
        <div slot="card">
            <el-card shadow="hover" v-if="$store.state.home_get.diary">
                ...
                <div class="sort_li">
                    <ul class="item">
                        <li v-for="value in sortTimeData" :key="value.id" >
                            <router-link :to="'/diary/pages/'+value.id" tag="span" @click.native="resetPage(value.id)">
...
export default {
    ...
    created() {
        let _this = this
        _this.$store.state.home_get.diary == null 
        ? _this.$store.dispatch('home_get/diary', '').then(()=>{
    ...
    methods: {
        ...
        saveLike:throttleTwo(function() {
            this.$store.dispatch('home_post/updateDiary', { id: '/'+this.$route.params.id , like_num: '0' } ).then(()=>{
                this.like_num ++
            }).catch(()=>{})},1000)
    }
}
